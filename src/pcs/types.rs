...
impl PublisherPcsData {
    ...
    pub fn pcs_from_nameplates<P: AsRef<Path>>(path: P) -> Result<HashMap<u16, PublisherPcsData>> {
        let configs = crate::pcs::nameplate::load_nameplates_from_csv(path)?;
        let mut out: HashMap<u16, PublisherPcsData> = HashMap::with_capacity(configs.len());
...
impl PublisherPcsData {
    ...
    /// Parses hex strings (e.g., "0x8801") or decimal strings to u16.
    pub fn nameplate_appid(&self) -> Option<u16> {
        self.nameplate.as_ref().and_then(|np| {
            np.goose_appid.as_ref().and_then(|appid_str| {
                let trimmed = appid_str.trim();
...
impl PublisherPcsData {
    /// Create a new PublisherPcsData with minimal required runtime fields. Nameplate
...
// ============================================================================
// GOOSE AppID Index and Mutable Data Structures
// ============================================================================

use dashmap::DashMap;

/// Index structure to map GOOSE APPID to logical PCS ID and type
/// Used for fast lookup when processing incoming GOOSE packets
#[derive(Debug, Clone)]
...
/// Thread-safe mutable PCS data storage
/// Uses DashMap for concurrent read/write access from multiple threads
#[derive(Debug)]
pub struct MutablePcsData {
    /// Primary storage indexed by logical_id for fast logical_id lookups in LAN1
    pub pcs_all_lan1: DashMap<u16, PublisherPcsData>,
    /// Primary storage indexed by logical_id for fast logical_id lookups in LAN2
    pub pcs_all_lan2: DashMap<u16, PublisherPcsData>,
}

impl MutablePcsData {
    /// Create new empty MutablePcsData
    pub fn new() -> Self {
        Self {
            pcs_all_lan1: DashMap::new(),
            pcs_all_lan2: DashMap::new(),
        }
    }
    ...
    pub fn update_with_index(
        ...
        header: &crate::goose::types::EthernetHeader,
        pdu: &IECGoosePdu,
        lan_id: u16,
    ) -> bool {
