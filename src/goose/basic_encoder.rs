...
pub fn encode_interger_general(
    tag: u8,
    value: &[u8],
    buffer: &mut [u8],
    pos: usize,
...
pub fn encode_unsigned_general(
    tag: u8,
    value: &[u8],
    buffer: &mut [u8],
    pos: usize,
...
pub fn encode_tag_length(
    ...
    if value < 128 {
        ...
        buffer[new_pos] = 0x81;  // 0x80 | 1 (1 byte follows)
        new_pos += 1;
        buffer[new_pos] = value as u8;
        new_pos += 1;
    } else if value < 65536 {
        // Long form: 2 bytes length
        buffer[new_pos] = 0x82;  // 0x80 | 2 (2 bytes follow)
        new_pos += 1;
        buffer[new_pos] = ((value >> 8) & 0xff) as u8;  // High byte
        new_pos += 1;
        buffer[new_pos] = (value & 0xff) as u8;  // Low byte - FIXED: was (value & 256)
        new_pos += 1;
    } else {
        // Long form: 3 bytes length
        buffer[new_pos] = 0x83;  // 0x80 | 3 (3 bytes follow)
        new_pos += 1;
        buffer[new_pos] = ((value >> 16) & 0xff) as u8;  // High byte
        new_pos += 1;
        buffer[new_pos] = ((value >> 8) & 0xff) as u8;   // Middle byte
        new_pos += 1;
        buffer[new_pos] = (value & 0xff) as u8;  // Low byte - FIXED: was (value & 256)
        new_pos += 1;
    }
