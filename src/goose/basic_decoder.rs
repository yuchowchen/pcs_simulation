...
pub fn decode_bit_string(
    ...
    *padding = buffer[new_pos];
    ...
    for i in 0..value.len() {
        ...
        value[value.len() - i - 1] = buffer[new_pos + i];
    }

    new_pos + length - 1 // by tornado chen @ 17th Jul 2022
}
pub fn decode_tag_length(tag: &mut u8, value: &mut usize, buffer: &[u8], pos: usize) -> usize {
    ...
    *tag = buffer[new_pos];
    ...
    match buffer[new_pos] {
        ...
        _ => {
            if buffer[new_pos] > 0x83 {
                // Log warning instead of panicking for unexpected length encoding
                error!("Warning: Unexpected length encoding at position {}: value 0x{:02x} exceeds maximum supported format (0x83)", new_pos, buffer[new_pos]);
                // Treat as single-byte length (safe default)
            }
            *value = buffer[new_pos] as usize;
            new_pos += 1;
        }
